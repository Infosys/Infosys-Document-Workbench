...
@Component
public class Rule005ExtractToAddressId extends AttributeExtractRuleAsyncBase {
	...
	public void doExtractAsync(List<Object> objList, IAttributeExtractRuleListener attributeExtractRuleListener) {
		...
		for (int i = 0; i < emailAddressToList.size(); i++) {
			...
			String toAddressId = emailAddressToList.get(i).getEmailId().toLowerCase();
			if (toAddressId != null && toAddressId.length() > 0) {
				attrValue += toAddressId;
		...
		attributeData.setAttrNameCde(EnumSystemAttributeName.TO_ADDRESS_ID.getCde()) // 5=To Address Id
