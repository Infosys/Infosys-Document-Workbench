...
@Component
public class Rule006ExtractCcAddress extends AttributeExtractRuleAsyncBase {
	...
	public void doExtractAsync(List<Object> objList, IAttributeExtractRuleListener attributeExtractRuleListener) {
		...
		for (int i = 0; i < emailAddressCcList.size(); i++) {
			String ccAddress = emailAddressCcList.get(i).getEmailName();
		...
		if (attrValue != null && attrValue.length() > 0) {
			...
			attributeData.setAttrNameCde(EnumSystemAttributeName.CC_ADDRESS.getCde()) // 6=CC Address
