...
@Component
@ActionExecutor(title = "Reject Passport", propertiesFile = "customization.properties")
public class RejectPassportActionExecutor extends ActionExecutorBase {
	...
	private static Logger logger = LoggerFactory.getLogger(RejectPassportActionExecutor.class);
	
	private static final String ACTION_RESULT_REJECTED = "Rejection Acknowledged";
	...
	protected boolean initialize(Properties properties) throws Exception {
		queueNameCdeList = Arrays.asList(properties.getProperty("action.reject-passport.queue.name.cde").split(","));
		actionNameCde = Integer.parseInt(properties.getProperty("action.reject-passport.action.name.cde"));
		return true;
	}
	...
	protected void executeAction(ActionParamAttrMappingData actionParamAttrMappingData,
		...
		String result = ACTION_RESULT_REJECTED;
