...
@Component
@ActionExecutor(title = "Approve Passport", propertiesFile = "customization.properties")
public class ApprovePassportActionExecutor extends ActionExecutorBase {
	...
	private static Logger logger = LoggerFactory.getLogger(ApprovePassportActionExecutor.class);
	
	private static final String ACTION_RESULT_APPROVED = "Approval Acknowledged";
	...
	protected boolean initialize(Properties properties) throws Exception {
		queueNameCdeList = Arrays.asList(properties.getProperty("action.approve-passport.queue.name.cde").split(","));
		actionNameCde = Integer.parseInt(properties.getProperty("action.approve-passport.action.name.cde"));
		return true;
	}
	...
	protected void executeAction(ActionParamAttrMappingData actionParamAttrMappingData,
		...
		String result = ACTION_RESULT_APPROVED;
