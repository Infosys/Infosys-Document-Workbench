...
@Component
@ActionExecutor(title = "Data Entry Reject", propertiesFile = "customization.properties")
public class DataEntryRejectExecutor extends ActionExecutorBase {
	...
	private static Logger logger = LoggerFactory.getLogger(DataEntryRejectExecutor.class);
	
	private static final String ACTION_RESULT_MSG = "Data Entry Rejected";
	...
	protected boolean initialize(Properties properties) throws Exception {
		queueNameCdeList = Arrays.asList(properties.getProperty("action.data-entry-reject.queue.name.cde").split(","));
		actionNameCde = Integer.parseInt(properties.getProperty("action.data-entry-reject.action.name.cde"));
		return true;
	}

	@Override
	protected List<List<DocActionData>> getActions() throws Exception {
		List<List<DocActionData>> docActionListOfList = actionService.getActionList(actionNameCde, EnumTaskStatus.YET_TO_START,
				queueNameCdeList, EnumEventOperator.EQUALS);
	...
	protected void executeAction(ActionParamAttrMappingData actionParamAttrMappingData,
		...
		String result = ACTION_RESULT_MSG;
	...
	protected EnumTaskStatus getActionCompleteTaskStatus(List<DocActionData> actionList)  throws Exception {
		return EnumTaskStatus.FOR_YOUR_REWORK;
	}
}
