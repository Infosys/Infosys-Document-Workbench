...
import com.infosys.ainauto.datasource.impl.emailserver.exchange.BaseTest;
import com.infosys.ainauto.datasource.impl.emailserver.exchange.common.TestHelper;
...
public class EmailServerDataSourceReaderTest extends BaseTest {
	...
	public static void setUpBeforeClass() throws Exception {
		emailServerDataSourceReaderConfig = TestHelper
				.getExchangeOnpremDataSourceReaderConfig("exchangeOnpremDataSourceTest.properties");
		try {
		...
		try {
			...
			// Filter for test email records
			{
				...
				while (iterator.hasNext()) {
					...
					try {
						if (!((EmailMessage) record.getMessage()).getSubject()
								.contains(TEST_RECORDS_IDENTIFIER_KEYWORD)) {
							iterator.remove();
						}
					} catch (Exception ex) {
