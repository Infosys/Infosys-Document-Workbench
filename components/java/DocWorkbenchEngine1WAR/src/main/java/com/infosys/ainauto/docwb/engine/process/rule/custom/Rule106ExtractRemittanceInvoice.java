		...
		try {
			...
			if (ruleType.isEmpty() || ruleType.equals(DocwbEngineCoreConstants.RULE_TYPE_DOCUMENT)) {
				...
				if (attachmentDataList != null && attachmentDataList.size() > 0) {
					...
					TableDataExtractorService.AttachmentData attachData = null;
					for (AttachmentData attachmentData : attachmentDataList) {
						attachData = new TableDataExtractorService.AttachmentData();
					...
					attachmentAttrDataList = new ArrayList<>();
					for (TableDataExtractorService.AttachmentData outputAttachData : invoiceAttributeDataList) {

						attributeDataList = new ArrayList<>();
						for (AttributeData outputAttrData : outputAttachData.getAttributes()) {
							if (ListUtility.hasValue(outputAttrData.getAttributeDataList())) {
								//Take the first one from the list which should be invoice number in this case
								String attrValue = outputAttrData.getAttributeDataList().get(0).getAttrValue();
								//String attrValue = outputAttrData.getAttrValue().replaceAll("[^A-Za-z0-9]", "").trim();
								attrValue = attrValue.replaceAll("[^A-Za-z0-9]", "").trim();
								attributeData = new AttributeData();
								attributeData.setAttrNameCde(EngineConstants.ATTR_NAME_CDE_INVOICE_NUMBERS) // 18=Invoice
																											// Number
										.setAttrValue(attrValue).setExtractType(EnumExtractType.CUSTOM_LOGIC)
										.setConfidencePct(outputAttrData.getConfidencePct());
								attributeDataList.add((attributeData));
								AttachmentData attachmentAttrData = new AttachmentData();
								attachmentAttrData.setAttachmentId(outputAttachData.getAttachmentId());
