...
public class Rule031ExtractAttachmentDocumentType extends AttributeExtractRuleAsyncBase {
	...
	public void doExtractAsync(List<Object> objList, IAttributeExtractRuleListener attributeExtractRuleListener) {
		...
		try {
			DocumentData document = (DocumentData) objList.get(0);
			DocumentData documentData = new DocumentData();
			BeanUtils.copyProperties(document, documentData);
			...
			if (attachmentDataList != null && attachmentDataList.size() > 0) {
				List<AttributeData> attributeFilterDataList = documentData.getAttributes().stream()
						.filter(data -> data.getAttrNameCde() == EnumSystemAttributeName.CATEGORY.getCde())
						.collect(Collectors.toList());
				List<AttachmentData> attachmentAttrDataList = new ArrayList<>();
				if (ListUtility.hasValue(attributeFilterDataList)) {
					AttributeData attrData = new AttributeData();
					BeanUtils.copyProperties(attributeFilterDataList.get(0), attrData);
					attrData.setAttrNameCde(EnumSystemAttributeName.DOCUMENT_TYPE.getCde()); // 31 = DocumentType
					attrData.setAttrNameTxt(null);
					for (AttachmentData attachmentData : attachmentDataList) {
						AttachmentData attachmentAttrData = new AttachmentData();
						List<AttributeData> attributeDataList = new ArrayList<>(Arrays.asList(attrData));
