...
@Component
@ActionExecutor(title = "Data Entry Complete", propertiesFile = "customization.properties")
public class DataEntryCompleteExecutor extends ActionExecutorBase {
	...
	private static Logger logger = LoggerFactory.getLogger(DataEntryCompleteExecutor.class);
	
	private static final String ACTION_RESULT_MSG = "Data Entry Completed";
	...
	protected boolean initialize(Properties properties) throws Exception {
		queueNameCdeList = Arrays.asList(properties.getProperty("action.data-entry-complete.queue.name.cde").split(","));
		actionNameCde = Integer.parseInt(properties.getProperty("action.data-entry-complete.action.name.cde"));
		return true;
	}
