...
@Component
@ActionExecutor(title = "Get Acknowledgement Number", propertiesFile = "customization.properties")
public class GetAcknowledgementNumberActionExecutor extends ActionExecutorBase {
	...
	private static Logger logger = LoggerFactory.getLogger(GetAcknowledgementNumberActionExecutor.class);
	...
	protected boolean initialize(Properties properties) throws Exception {
		queueNameCdeList = Arrays
				.asList(properties.getProperty("action.get-acknowledgement-number.queue.name.cde").split(","));
		actionNameCde = Integer.parseInt(properties.getProperty("action.get-acknowledgement-number.action.name.cde"));
		return true;
	}
	...
	protected void executeAction(ActionParamAttrMappingData actionParamAttrMappingData,
		...
		long number = (long) Math.floor(StringUtility.getRandomNumberInDouble() * 9_000_000_000L) + 1_000_000_000L;
