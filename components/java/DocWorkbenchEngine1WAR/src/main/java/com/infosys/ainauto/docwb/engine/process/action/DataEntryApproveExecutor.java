...
@Component
@ActionExecutor(title = "Data Entry Approve", propertiesFile = "customization.properties")
public class DataEntryApproveExecutor extends ActionExecutorBase {
	...
	private static Logger logger = LoggerFactory.getLogger(DataEntryApproveExecutor.class);
	
	private static final String ACTION_RESULT_MSG = "Data Entry Approved";
	...
	protected boolean initialize(Properties properties) throws Exception {
		queueNameCdeList = Arrays.asList(properties.getProperty("action.data-entry-approve.queue.name.cde").split(","));
		actionNameCde = Integer.parseInt(properties.getProperty("action.data-entry-approve.action.name.cde"));
		return true;
	}
