...
class Rule1000CusPropertyEmailBodyRefno(RulePropertyBaseClass):
    ...
    def extract(self, doc_data_dict: dict) -> RESPONSE_DICT:
        ...
        try:
            ...
            match_val = re.search(
                "ref.* (\d+)", EmailParserService(doc_file_path).get_body())
            response_val_dict["text"] = match_val.group(
                1) if match_val else None
