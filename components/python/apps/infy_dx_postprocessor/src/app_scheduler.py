...
class AppArgs:
    def __init__(self) -> None:
        ...
        self.__extractor_outfile_path = None
        self.data_file_path = None
        self.extractor_multi_outfile_path = None
    ...
    @property
    def extractor_outfile_path(self):
        return self.__extractor_outfile_path

    @extractor_outfile_path.setter
    def extractor_outfile_path(self, value):
        self.__extractor_outfile_path = value
...
class AppScheduler:
    ...
    def __get_app_args(self):
        ...
        for stage_file in FileUtil.get_files(
            ...
            req_file_list = FileUtil.get_files(master_config_data["extractor"]["output_path_root"],
                                               os.path.basename(stage_file).replace(".txt", ""))
            if req_file_list:
                app_args.request_id = os.path.basename(
                    req_file_list[0]).split("_")[0]
                app_args.extractor_outfile_path = req_file_list[0]
    ...
    def __schedule_helper(self):
        ...
        if not app_args.extractor_outfile_path:
