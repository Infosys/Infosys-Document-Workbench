...
DOC_TYPE_TO_ATTR_MAP: dict = {
    "Invoice": ["invoicenum", "statementperiod"],
    "Annual Report": ["financial_year"],
    "Statement of Work":["project_name"]
}


class RuleSysDocumentType(RuleAttributeBaseClass):
    ...
    def extract(self, raw_attribute_data_list: list, business_attribute_data: dict) -> BIZ_ATTRIBUTE_DATA_DICT:
        try:
            ...
            for raw_attribute_data in raw_attribute_data_list:
                attr_value_temp = ''
                for key, vals in DOC_TYPE_TO_ATTR_MAP.items():
