...
:: ##### Drive mapping #####
::SET DRIVE_LETTER=G
::SET PATH_TO_MAP=\\infosys\data
::ECHO.
::ECHO Checking if drive [%DRIVE_LETTER%:] exists
::VOL %DRIVE_LETTER%: >nul 2>nul
::IF %ERRORLEVEL% EQU 0 (
::	ECHO Drive [%DRIVE_LETTER%:] found. Mapping is skipped.
::) ELSE (
::	ECHO Drive [%DRIVE_LETTER%:] not found. Mapping...	
::	net use %DRIVE_LETTER%: %PATH_TO_MAP% /persistent:yes
::)
:: ##### Drive mapping #####
